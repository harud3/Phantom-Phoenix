...
public class TensionMovement : MonoBehaviour, IPointerDownHandler
{
    ...
    IEnumerator waitPlayerClick(TensionController tc)
    {
        ...
        if (TargetCheck(clickedGameObject) is var x && x.passed)
        {
            if(x.cctarget != null)
            {
    ...
    /// <returns></returns>
    private (bool passed, HeroController hctarget, CardController cctarget, int targetFieldIDByReceiver) TargetCheck(GameObject clickGameObject)
    {
